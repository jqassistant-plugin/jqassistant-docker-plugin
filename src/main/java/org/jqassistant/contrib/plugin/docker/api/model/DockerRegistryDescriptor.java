...
public interface DockerRegistryDescriptor extends DockerDescriptor {
    ...
    @ResultOf
    @Cypher("MATCH (registry:Docker:Registry) WHERE id(registry)=$this MERGE (registry)-[:CONTAINS_BLOB]->(blob:Docker:Blob{digest:$digest}) RETURN blob")
    DockerBlobDescriptor resolveBlob(@Parameter("digest") String name);
}
